...
def _get_unique_pdfs_in_docs(docs: list[dict]) -> set[str]:
    return {doc.get("metadata", {}).get("pdf", "unknown") for doc in docs}
...
11. Context comes from MULTIPLE papers. You MUST cover EVERY paper listed above —
    ONLY those papers, no others. Do NOT mention or reference any paper not in the list above.
    Structure your answer with EXACTLY this format for each paper — no exceptions:

    **filename.pdf**: [your answer for this paper]
    CONFIDENCE: High / Medium / Low

    Every single paper block MUST end with its own CONFIDENCE line immediately after the answer.
    Do NOT put a single global CONFIDENCE at the end. Do NOT skip the CONFIDENCE for any paper.
    Do NOT split one paper into multiple blocks. Do NOT skip any paper.
    After all per-paper blocks, add a brief overall comparison if relevant.
12. If a paper has no relevant context: write exactly:
    **filename.pdf**: The retrieved context does not contain enough information.
    CONFIDENCE: Low
13. Only flag a conflict if two papers make DIRECTLY OPPOSING factual claims.
    ...
    Do NOT flag a conflict if papers discuss different topics or one has no information."""
...
def generate_answer(
    ...
    papers_line = (
        f"Papers in context ({len(pdf_names)}): {', '.join(pdf_names)}\n\n"
        if is_multi_pdf and pdf_names else ""
    )
...
def ask_question(query: str, doc_id: str | None = None) -> tuple[str, list[dict], list[str], int]:
    ...
    present_pdf_names = sorted(_get_unique_pdfs_in_docs(top_docs)) if is_multi_pdf else None
...
def _business_followup_suggestions(query: str, answer: str) -> list[str]:
    ...
    from config import GROQ_MODEL_SMALL
