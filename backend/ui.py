...
from backend.pipeline.loader import load_pdf, extract_images
from backend.pipeline.ocr import caption_images
from backend.pipeline.indexer import add_to_index, get_index, load_state, remove_from_index
from backend.pipeline.chunker import chunk_text_with_pages, create_documents, clean_documents
from backend.pipeline.db import (
    ...
    with gr.Blocks(
        ...
        with gr.Tab("Eval"):
            ...
            def _refresh_dashboard():
                from backend.pipeline.db import get_eval_scores, get_eval_summary
            ...
            def run_scoring(n: int):
                from backend.eval.run_eval import run_on_recent_queries
            ...
            def show_chunks_for_row(evt: gr.SelectData, failures_df):
                try:
                    ...
                    from backend.pipeline.db import get_eval_scores
                    rows  = get_eval_scores(limit=200)
            ...
            def clear_scores():
                from backend.pipeline.db import clear_eval_scores
                clear_eval_scores()
